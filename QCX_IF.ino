...
//  H45minus.begin(h45m,HILBERT_IF);
  ...
  BFO.frequency(7700);                     // put on upper band edge of the Roofing filter
                                           // adjust for best sound vs image rejection
                                           // no commands for the 2nd mixer, it just works
...
int i,mult;
   ...
   if( (vfo_mode & VFO_USB ) ) vfo -= 7000;             // !!! needs work here
   if( (vfo_mode & VFO_LSB ) ) vfo += 7000;
   if( (vfo_mode & VFO_CW ) &&  mode_menu_data.current == 1) vfo -= 7700;
